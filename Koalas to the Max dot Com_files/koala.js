...
(function() {
  ...
  Rect.prototype.isSplitable = function() {
  ...
  Rect.prototype.split = function() {
    ...
    delete this.node;
    Rect.addToVis(this.vis, this.children);
    this.onSplit(this);
  }

  Rect.prototype.checkIntersection = function(startPoint, endPoint) {
    ...
    // End point is inside the rect and start point is outside
    return edx * edx + edy * edy <= r2 && sdx * sdx + sdy * sdy > r2;
  }

  Rect.addToVis = function(vis, circles, init) {
    var rect = vis.selectAll('.nope').data(circles)
      .enter().append('rect');

    if (init) {
      // Setup the initial state of the initial rect
      rect = rect
      ...
      rect = rect
        ...
        .attr('width',    function(d) { return d.parent.size / 2; })
        .attr('height',    function(d) { return d.parent.size / 2; })
  ...
  koala.makeCircles = function(selector, colorData, onEvent) {
    ...
    function onSplit(rect) {
      // manage events
      var layer = rect.layer;
    ...
    for (yi = 0; yi < dim; yi++) {
      for (xi = 0; xi < dim; xi++) {
        ...
        finestLayer(xi, yi, new Rect(vis, xi, yi, size, color));
        t += 4;
      }
    }
      ...
      for (yi = 0; yi < dim; yi++) {
        for (xi = 0; xi < dim; xi++) {
          ...
          c1.parent = c2.parent = c3.parent = c4.parent = layer(xi, yi,
            new Rect(vis, xi, yi, size, color, [c1, c2, c3, c4], currentLayer, onSplit)
          );
        }
      }
    ...
    Rect.addToVis(vis, [layer(0, 0)], true);

    // Interaction helper functions
    function splitableCircleAt(pos) {
      var xi = Math.floor(pos[0] / minSize),
          yi = Math.floor(pos[1] / minSize),
          rect = finestLayer(xi, yi);
      if (!rect) return null;
      while (rect && !rect.isSplitable()) rect = rect.parent;
      return rect || null;
    }
    ...
    function findAndSplit(startPoint, endPoint) {
      ...
      for (var i = 0; i < breaks.length - 1; i++) {
        ...
        var rect = splitableCircleAt(ep);
        if (rect && rect.isSplitable() && rect.checkIntersection(sp, ep)) {
          rect.split();
        }
      }
    }
